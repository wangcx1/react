 ...
 * @param {string} location e.g. "prop", "context", "pc_index context"
...
_reactDom2.default.render(_react2.default.createElement(Hello, null), document.getElementById('header'));
...
if (process.env.NODE_ENV !== "production") {
  (function () {
    ...
    function defineKeyPropWarningGetter(props, displayName) {
      var warnAboutAccessingKey = function warnAboutAccessingKey() {
        if (!specialPropKeyWarningShown) {
          ...
          warning(false, '%s: `key` is not a prop. Trying to access it will result ' + 'in `undefined` being returned. If you need to access the same ' + 'value within the pc_index component, you should pass it as a different ' + 'prop. (https://fb.me/react-special-props)', displayName);
        }
      };
    ...
    function defineRefPropWarningGetter(props, displayName) {
      var warnAboutAccessingRef = function warnAboutAccessingRef() {
        if (!specialPropRefWarningShown) {
          ...
          warning(false, '%s: `ref` is not a prop. Trying to access it will result ' + 'in `undefined` being returned. If you need to access the same ' + 'value within the pc_index component, you should pass it as a different ' + 'prop. (https://fb.me/react-special-props)', displayName);
        }
      };
    ...
    /**
     * TODO: Test that a single pc_index and an array with one item have the same key
     * pattern.
     */
     ...
     * @param {!function} callback Callback to invoke with each pc_index found.
    ...
    function traverseAllChildrenImpl(children, nameSoFar, callback, traverseContext) {
      ...
      if (invokeCallback) {
        ...
        // If it's the only pc_index, treat the name as if it was wrapped in an array
      ...
      if (Array.isArray(children)) {
        ...
        if (typeof iteratorFn === 'function') {
          ...
          invariant(false, 'Objects are not valid as a React pc_index (found: %s).%s', childrenString === '[object Object]' ? 'object with keys {' + Object.keys(children).join(', ') + '}' : childrenString, addendum);
        }
      }
     ...
     * @param {!function} callback To invoke upon traversing each pc_index.
     ...
     *
     * The provided forEachFunc(pc_index, index) will be called for each
     * leaf pc_index.
     *
     * @param {?*} children Children tree container.
     ...
     *
     * The provided mapFunction(pc_index, key, index) will be called for each
     * leaf pc_index.
     *
     * @param {?*} children Children tree container.
    ...
    /**
     * Returns the first pc_index in a collection of children and verifies that there
     * is only one pc_index in the collection.
     *
     * See https://reactjs.org/docs/react-api.html#react.children.only
     *
     * The current implementation of this function assumes that a single pc_index gets
    ...
    function onlyChild(children) {
      !isValidElement(children) ? invariant(false, 'React.Children.only expected to receive a single React element pc_index.') : void 0;
      return children;
    }
    ...
    function validateExplicitKey(element, parentType) {
      ...
      // property, it may be the creator of the pc_index that's responsible for
      // assigning it a key.
      var childOwner = '';
      if (element && element._owner && element._owner !== ReactCurrentOwner.current) {
        // Give the component that originally created this pc_index.
        childOwner = ' It was passed a pc_index from ' + getComponentName(element._owner) + '.';
      }

      currentlyValidatingElement = element;
      {
        warning(false, 'Each pc_index in an array or iterator should have a unique "key" prop.' + '%s%s See https://fb.me/react-warning-keys for more information.%s', currentComponentErrorInfo, childOwner, getStackAddendum());
      }
     ...
     *
     * @internal
     * @param {ReactNode} node Statically passed pc_index of any type.
...
break;}// If both copies of the parent fiber point to the same pc_index, we can
// assume that the pc_index is current. This happens when we bailout on low
// priority: the bailed out fiber's pc_index reuses the current pc_index.
...
// belong to the pc_index set of A.return, and B must belong to the pc_index
// set of B.return.
a=parentA;b=parentB;}else{// The return pointers point to the same fiber. We'll have to use the
// default, slow path: scan the pc_index sets of each parent alternate to see
// which pc_index belongs to which set.
//
// Search parent A's pc_index set
var didFindChild=false;var _child=parentA.child;while(_child){if(_child===a){didFindChild=true;a=parentA;b=parentB;break;}if(_child===b){didFindChild=true;b=parentA;a=parentB;break;}_child=_child.sibling;}if(!didFindChild){// Search parent B's pc_index set
...
// we have already pushed its own pc_index context on the stack. A context
// provider should not "see" its own pc_index context. Therefore we read the
...
// assuming that after the first pc_index we've already added everything.
...
// to forget to do before returning it. E.g. for the single pc_index case.
var clone=createWorkInProgress(fiber,pendingProps,expirationTime);clone.index=0;clone.sibling=null;return clone;}function placeChild(newFiber,lastPlacedIndex,newIndex){newFiber.index=newIndex;if(!shouldTrackSideEffects){// Noop.
return lastPlacedIndex;}var current=newFiber.alternate;if(current!==null){var oldIndex=current.index;if(oldIndex<lastPlacedIndex){// This is a move.
newFiber.effectTag=Placement;return lastPlacedIndex;}else{// This item can stay in place.
return oldIndex;}}else{// This is an insertion.
newFiber.effectTag=Placement;return lastPlacedIndex;}}function placeSingleChild(newFiber){// This is simpler for the single pc_index case. We only need to do a
...
// need to delete the existing pc_index.
...
// it from the pc_index list so that we don't add it to the deletion
// list.
...
// the rest.
deleteRemainingChildren(returnFiber,currentFirstChild.sibling);var existing=useFiber(currentFirstChild,textContent,expirationTime);existing['return']=returnFiber;return existing;}// The existing first pc_index is not a text node so we need to create one
// and delete the existing ones.
deleteRemainingChildren(returnFiber,currentFirstChild);var created=createFiberFromText(textContent,returnFiber.internalContextTag,expirationTime);created['return']=returnFiber;return created;}function reconcileSingleElement(returnFiber,currentFirstChild,element,expirationTime){var key=element.key;var child=currentFirstChild;while(child!==null){// TODO: If key === null and pc_index.key === null, then this only applies to
// the first item in the list.
if(child.key===key){if(child.tag===Fragment?element.type===REACT_FRAGMENT_TYPE:child.type===element.type){deleteRemainingChildren(returnFiber,child.sibling);var existing=useFiber(child,element.type===REACT_FRAGMENT_TYPE?element.props.children:element.props,expirationTime);existing.ref=coerceRef(child,element);existing['return']=returnFiber;{existing._debugSource=element._source;existing._debugOwner=element._owner;}return existing;}else{deleteRemainingChildren(returnFiber,child);break;}}else{deleteChild(returnFiber,child);}child=child.sibling;}if(element.type===REACT_FRAGMENT_TYPE){var created=createFiberFromFragment(element.props.children,returnFiber.internalContextTag,expirationTime,element.key);created['return']=returnFiber;return created;}else{var _created7=createFiberFromElement(element,returnFiber.internalContextTag,expirationTime);_created7.ref=coerceRef(currentFirstChild,element);_created7['return']=returnFiber;return _created7;}}function reconcileSingleCall(returnFiber,currentFirstChild,call,expirationTime){var key=call.key;var child=currentFirstChild;while(child!==null){// TODO: If key === null and pc_index.key === null, then this only applies to
// the first item in the list.
...
// won't update its pc_index set by applying minimal side-effects. Instead,
// we will add them all to the pc_index before it gets rendered. That means
// we can optimize this reconciliation pass by not tracking side-effects.
workInProgress.child=mountChildFibers(workInProgress,null,nextChildren,renderExpirationTime);}else{// If the current pc_index is the same as the work in progress, it means that
...
// During mounting we don't know the pc_index context yet as the instance doesn't exist.
// We will invalidate the pc_index context in finishClassComponent() right after rendering.
...
// we don't do the bailout and we have to reuse existing props instead.
}else if(nextProps===null||memoizedProps===nextProps){return bailoutOnAlreadyFinishedWork(current,workInProgress);}var nextChildren=nextProps.children;var isDirectTextChild=shouldSetTextContent(type,nextProps);if(isDirectTextChild){// We special case a direct text pc_index of a host node. This is a common
// case. We won't handle it as a reified pc_index. We will instead handle
// this in the host environment that also have access to this prop. That
// avoids allocating another HostText fiber and traversing it.
nextChildren=null;}else if(prevProps&&shouldSetTextContent(type,prevProps)){// If we're switching from a direct text pc_index to a normal pc_index, or to
...
// eagerly do the work of workInProgress.pc_index as an optimization.
...
// the root always starts with a "current" with a null pc_index.
...
workInProgress.child=reconcileChildFibers(workInProgress,null,nextChildren,renderExpirationTime);memoizeProps(workInProgress,nextChildren);}else{reconcileChildren(current,workInProgress,nextChildren);memoizeProps(workInProgress,nextChildren);}return workInProgress.child;}/*
  function reuseChildrenEffects(returnFiber : Fiber, firstChild : Fiber) {
    let pc_index = firstChild;
    do {
      // Ensure that the first and last effect of the parent corresponds
      // to the children's first and last effect.
      if (!returnFiber.firstEffect) {
        returnFiber.firstEffect = pc_index.firstEffect;
      }
      if (pc_index.lastEffect) {
        if (returnFiber.lastEffect) {
          returnFiber.lastEffect.nextEffect = pc_index.firstEffect;
        }
        returnFiber.lastEffect = pc_index.lastEffect;
      }
    } while (pc_index = pc_index.sibling);
  }
...
//   if (current && current.pc_index !== workInProgress.pc_index) {
//     reuseChildrenEffects(workInProgress, pc_index);
//   }
//   return null;
// }
...
// deletions," reset the pc_index to the current pc_index to make sure we delete
...
var node=workInProgress.child;while(node!==null){if(node.tag===HostComponent||node.tag===HostText){appendInitialChild(parent,node.stateNode);}else if(node.tag===HostPortal){// If we have a portal pc_index, then we don't want to traverse
// down its children. Instead, we'll get insertions from each pc_index in
// the portal directly.
...
// children to find all the terminal nodes.
var node=workInProgress.child;while(node!==null){if(node.tag===HostComponent||node.tag===HostText){appendChildToContainerChildSet(containerChildSet,node.stateNode);}else if(node.tag===HostPortal){// If we have a portal pc_index, then we don't want to traverse
// down its children. Instead, we'll get insertions from each pc_index in
// the portal directly.
...
// should clear the pc_index pointer of the parent alternate to let this
// get GC:ed but we don't know which for sure which parent is the current
// one so we'll settle for GC:ing the subtree of this pc_index. This pc_index
...
if(node.effectTag&Placement){// If we don't have a pc_index, try the siblings instead.
continue siblings;}// If we don't have a pc_index, try the siblings instead.
...
// new markup in a temp node and then move the pc_index nodes across into
// the target node
...
// TODO: Warn if there is more than a single textNode as a pc_index.
...
// https://html.spec.whatwg.org/multipage/syntax.html#parsing-main-inbody
switch(tag){case'h1':case'h2':case'h3':case'h4':case'h5':case'h6':return parentTag!=='h1'&&parentTag!=='h2'&&parentTag!=='h3'&&parentTag!=='h4'&&parentTag!=='h5'&&parentTag!=='h6';case'rp':case'rt':return impliedEndTags.indexOf(parentTag)===-1;case'body':case'caption':case'col':case'colgroup':case'frame':case'head':case'html':case'tbody':case'td':case'tfoot':case'th':case'thead':case'tr':// These tags are only valid with a few parents that have special pc_index
